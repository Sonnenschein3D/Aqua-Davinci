...
export function setupMachine(
    ...
    const machineBrush = new BrushRenderer(scene);

// Listen for updates to the 3D brush model
eventBus.on('request-3d-brush-update', (state: any) => {
    machineBrush.update(state); 
});
    ...
    eventBus.on('save-brush-profile', (obj: THREE.Object3D) => {
        ...
        if (rawSegments.length > 0) {
            ...
            if (nodes && helpers) {
                ...
                if (obj.userData.materialParams?.opacity !== undefined) {
                    currentBrushState.transparency = obj.userData.materialParams.opacity;
                }
                if (obj.userData.baseSize) {
                    if (obj.userData.baseSize.radius !== undefined) currentBrushState.diameter = obj.userData.baseSize.radius * 2;
                    if (obj.userData.baseSize.height !== undefined) currentBrushState.length = obj.userData.baseSize.height;
                }
                ...
                // WICHTIG: Sende die aktualisierten Werte sofort an das UI (PropertiesPanel) zurück!
                eventBus.emit('brush-state-changed', currentBrushState);
            }
        }
    });
