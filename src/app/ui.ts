    ...
    eventBus.on('tool-selected', (id: string) => {
        if (id === 'brush_create') {
            eventBus.emit('request-brush-state-sync', (brushState: any) => {
                 if (brushState) {
                    // Alte Pinselvorlagen von der Arbeitsfläche entfernen
                    const objects = objectManager.getObjects();
                    const oldBrushes = objects.filter(o => o.userData.isBrushTemplate);
                    oldBrushes.forEach(oldBrush => {
                        objectManager.removeObject(oldBrush);
                    });
                    ...
                    // Immer mit 'Neuer Pinsel' starten
                    brush.name = 'Neuer Pinsel';
