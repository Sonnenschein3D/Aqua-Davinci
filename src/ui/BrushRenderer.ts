...
export class BrushRenderer {
    ...
    constructor(scene: THREE.Scene) {
        ...
        this.group.name = 'BrushRenderer_Group'; // Changed group name as well
    ...
    public update(state: any, currentPos?: THREE.Vector3, movementDelta?: THREE.Vector3) {
        ...
        if (state.bezierProfile) {
            ...
            // Ensure bezierProfile is structured correctly if loaded from JSON

            // For example, if loaded as plain objects, convert them back to THREE.Vector2

            if (state.bezierProfile.p1 && !(state.bezierProfile.p1 instanceof THREE.Vector2)) {

                state.bezierProfile.p1 = new THREE.Vector2(state.bezierProfile.p1.x, state.bezierProfile.p1.y);

            }

            if (state.bezierProfile.cp1 && !(state.bezierProfile.cp1 instanceof THREE.Vector2)) {

                state.bezierProfile.cp1 = new THREE.Vector2(state.bezierProfile.cp1.x, state.bezierProfile.cp1.y);

            }

            if (state.bezierProfile.cp2 && !(state.bezierProfile.cp2 instanceof THREE.Vector2)) {

                state.bezierProfile.cp2 = new THREE.Vector2(state.bezierProfile.cp2.x, state.bezierProfile.cp2.y);

            }

            if (state.bezierProfile.p0 && !(state.bezierProfile.p0 instanceof THREE.Vector2)) {

                state.bezierProfile.p0 = new THREE.Vector2(state.bezierProfile.p0.x, state.bezierProfile.p0.y);
            ...
            // Reset to default profile if bezierProfile is null or not provided
